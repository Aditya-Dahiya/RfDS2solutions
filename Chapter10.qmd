...
The code shown below recreates the @fig_q1_ex5 using the function `geom_pointrange()` instead of the easier and shorter `stat_summary()` .
...
Another example showing the need for use of `group = 1` is below in @fig-q5a-ex5 . This plot again calculates the proportion of observations within each group formed by unique combinations of `cut` and `color` .
...
```{r}
#| label: fig-q5a-ex5
#| fig-cap: "Use of group=1 in coloured bar-plot to resolve the error"
...
This plot (shown in @fig-q1-ex6) suffers from over-plotting, since there are only 78 points displayed, against a total of 234 observations. So, we don't know if there are multiple observations of one type or many tpes, and where are the most observations located.
...
**What\'s the default position adjustment for [`geom_boxplot()`](https://ggplot2.tidyverse.org/reference/geom_boxplot.html)? Create a visualization of the `mpg` dataset that demonstrates it.**
